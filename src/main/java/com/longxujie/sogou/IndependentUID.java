...
public class IndependentUID extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        FileOutputFormat.setOutputPath(job1, new Path("/mr/analysis_sougoulog/data/sogou_independentUID"));
        job1.waitForCompletion(true);

        Job job2=Job.getInstance(conf);

        job2.setJarByClass(IndependentUID.class);
        FileInputFormat.addInputPath(job2, new Path("/mr/analysis_sougoulog/data/sogou_independentUID"));
        job2.setMapperClass(IndependentUIDMapper2.class);
        job2.setReducerClass(IndependentUIDReducer2.class);
        job2.setOutputKeyClass(Text.class);
        job2.setOutputValueClass(LongWritable.class);
        FileOutputFormat.setOutputPath(job2, new Path("/mr/analysis_sougoulog/data/4_IndependentUID"));
